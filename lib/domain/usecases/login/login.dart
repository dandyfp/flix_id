...
class Login implements UseCase<Result<User>, LoginParams> {
  ...
  Future<Result<User>> call(LoginParams params) async {
    ...
    if (idResult is Success) {
      ...
      return switch (userResult) { Success(value: final user) => Result.success(user), Failed(:final message) => Result.failed(message) };
    } else {
      return Result.failed(idResult.errorMessagge!);
    }
  }
}
